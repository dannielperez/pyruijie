...
- Monitoring-center hub is a **dual-WAN** Ruijie EG (LAN `***REMOVED***`). Sites run a WG **client**
  policy (`WG_CLIENT`) to the hub.
- **Site endpoint should be a DDNS, not a static WAN IP.** A static IP (e.g. `***REMOVED***`) dies
  with that WAN; the DDNS (`***REMOVED***`) follows whichever WAN is live. This was
...
## Suggested high-level helpers (would directly back downstream application actions)
